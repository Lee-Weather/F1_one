...
- **RL 训练代码只能从 GitHub 获取**：必须先将训练 commit 推送到 GitHub 并核对远端 SHA，再创建/启动训练任务；确认任务已进入运行态后，才允许将同一 commit 推送到 GitLab 镜像。GitLab 不得作为训练任务的 `codeUrl`。完整规则见 [references/git-mirror.md](references/git-mirror.md)。
...
## RL 训练代码源与 Git 镜像顺序（强制）

以下顺序适用于所有通过 `flux task` 创建或恢复的 RL 训练版本：

1. 确认训练使用的分支、commit SHA 和工作区状态；训练 commit 必须已经提交，不能用未提交的工作区内容作为版本依据。
2. 先将该分支的同一 commit 推送到 GitHub `origin`，再用 `git ls-remote --heads origin <branch>` 核对远端 SHA。GitHub 推送失败、远端 SHA 不一致、发生非 fast-forward 或需要 `--force` 时，停止流程，不创建也不启动训练任务。
3. `taskCodeInfo.codeUrl` 必须填写 GitHub 仓库地址和对应分支；不得填写 GitLab 地址。`mainCodeUri`、`hparamsPath` 和 `startScript` 必须按 GitHub 仓库克隆后的目录结构填写。
4. 对 `flux task create` 先做 `--dry-run`，正式创建后执行 `flux task run`；通过 `flux task info` 确认任务已经进入运行态（状态码 `3`）。
5. 只有在确认任务已启动后，才将同一分支、同一 commit 推送到 GitLab `gitlab` 远程，并用 `git ls-remote --heads gitlab <branch>` 核对 SHA。GitLab 仅作训练版本镜像，不参与本次训练代码拉取。
6. GitLab 镜像推送失败时，记录并报告失败原因，但不要因镜像失败自动停止已经运行的训练任务；GitHub 推送或任务启动确认失败时，不得继续进入 GitLab 镜像步骤。

完整执行顺序和去重规则见 [references/git-mirror.md](references/git-mirror.md)。
...
### 私有 GitHub 仓库需要配置账号和 Token

> **重要提示：RL 训练任务只能使用 GitHub 代码源；当用户使用私有 GitHub 仓库创建训练任务时，必须确认已在平台配置 GitHub 凭证。**

背景：
- 任务运行时，后端会自动从用户资料中读取 `github_name` + `github_token` 来拉取私有 GitHub 仓库代码。
- 这些凭证通过 Web 平台的「个人设置 → Git 信息」页面配置（对应后端接口 `POST /api/user/editGitInfo`）。
- **flux-cli 当前没有管理 Git 凭证的命令**，无法通过 CLI 直接设置。

**Agent 应遵循以下规则：**
1. 当用户提供的 `codeUrl` 为私有 GitHub 仓库时，**必须主动询问**用户是否已在平台配置 GitHub 账号和 Token。
